...
//        glm::vec3 out_camera_pos = glm::vec3(0.0f, 0.0f, 0.0f);
...
//
//
//bool Basic3D::createScene(float out_vertices[], int out_vertices_arr_len) {
////    glfw init and configurate
//    if(!createWindow()) {
//        cout << "Location :: File(basic_3D.cpp), Funtion(createScene), Error :: createWindow error" << endl;
//        return false;
//    }
////    glad configurate
//    if(setGLAD() == -1) {
//        cout << "Location :: File(basic_3D.cpp), Function(setGLAD()), Error :: setGLAD failed." << endl;
//        return false;
//    }
////    启用深度测试
//    glEnable(GL_DEPTH_TEST);
...
//    if(!setShaderProgram()) {
//        cout << "Location :: File(basic_3D.cpp), Function(setShaderProgram()), Error :: setShaderProgram failed." << endl;
//        return false;
//    }
////    prepare data buffer
//    prepareDataBuffer(out_vertices, out_vertices_arr_len);
...
////    绑定VBO : 只需要绑定VBO不用再次设置VBO的数据，因为箱子的VBO数据中已经包含了正确的立方体顶点数据
//    glBindBuffer(GL_ARRAY_BUFFER, VBO);
////    设置灯光的顶点属性
//    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void *)0);
//    glEnableVertexAttribArray(0);
//    
////    为fragment shader设定颜色
//    glUseProgram(shaderProgram);
